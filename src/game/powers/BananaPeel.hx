...
class BananaPeel extends Entity
{
	...
	public function new(x:Float, y:Float, _dir:Float)
	{
		...
		sprite = new Spritemap("gfx/powers/bananapeel.png", 8, 8);
	...
	override public function update()
	{
		...
		/*for (i in 0...Types.types.length) {
				if (t != Types.types[i]) {
					var _wiz = collide(Types.types[i], x, y);

					if (_wiz != null) {
						cast(_wiz, Player).bananaTimer = 120;
						cast(_wiz, Player).velocity.x = ((cast(_wiz, Player).facing == 0) ? -1 : 1) * cast(_wiz, Player).bananaMaxVel.x;
					}
				}
			}
		}*/
