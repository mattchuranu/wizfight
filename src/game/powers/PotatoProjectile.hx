...
class PotatoProjectile extends Entity
{
	...
	public function new(x:Float, y:Float, _vel:Float, _t:String)
	{
		super(x, y);

		sprite = new Spritemap("gfx/shot/potatowizshot.png", 6, 6);
		...
		gravity = new Point(0, 0.5); //original x = 0, y = 1.8
		maxVel = new Point(2, 5); //original x = 2, y = 12

		var mult = (dir == 0) ? -1 : 1;
		velocity = new Point(_vel, 0);
